  ...
  return createPortal(
    ...
    >
      <div
        ...
        aria-label={title}
        className={cn(
          "my-8 w-full max-w-2xl overflow-hidden rounded-2xl border border-white/10 bg-card shadow-2xl shadow-black/60",
          className,
        )}
      >
        <header className="flex items-center justify-between border-b border-white/5 px-5 py-3">
          ...
          <button
            ...
            onClick={onClose}
            aria-label="Close"
            className="rounded-md p-1 text-muted-foreground transition hover:bg-white/5 hover:text-foreground"
          >
            <X className="h-4 w-4" />
          </button>
        </header>
