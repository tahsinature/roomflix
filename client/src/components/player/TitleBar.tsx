...
export function TitleBar({ title }: Props) {
  return (
    <VControls.Root className="pointer-events-none absolute inset-x-0 top-0 z-20 opacity-0 transition-opacity duration-200 ease-out data-[visible]:opacity-100">
      ...
      <VControls.Group className="pointer-events-auto relative px-3 pt-2 sm:px-4 sm:pt-3">
        <h2 className="line-clamp-1 text-sm font-medium text-white/90 sm:text-base" title={title}>
          {title}
        </h2>
      </VControls.Group>
    </VControls.Root>
